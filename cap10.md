...
Quando usamos CI, o master é constantemente atualizado com código novo. Para garantir que ele continue consistente, algumas práticas são importantes quando se usa CI. Vamos discutir algumas delas a seguir.

#### Build Automatizado {.unnumbered}

**Build** é o termo usado para designar a compilação de todo os arquivos de um sistema, até a geração de uma versão executável. Quando se usa CI, o build deve ser totalmente automatizado, isto é, não incluir nenhum passo manual. Além disso, é importante que ele seja o mais rápido possível, pois com integração contínua ele será sempre executado. Alguns autores, por exemplo, chegam a recomendar um limite de 10 minutos para conclusão de builds ([link](https://dl.acm.org/doi/book/10.5555/318762)). 

#### Testes Automatizados {.unnumbered}

Após cada commit, é importante garantir que o master continue com o comportamento esperado. Por isso, ao usar CI, deve-se ter uma boa cobertura de testes, principalmente testes de unidade, os quais já foram estudados no Capítulo 8. 
...
* No Facebook, "todos os engenheiros de front-end trabalham em um único branch que é mantido estável, o que torna o desenvolvimento mais rápido, pois não dispende-se esforço na integração de branches de longa duração no trunk." ([link](https://doi.org/10.1109/MIC.2013.25))
```{=latex}
\end{esmbox}
```
