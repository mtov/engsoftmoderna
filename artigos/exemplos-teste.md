...
concorrência, cache, E/S e grafos. Ela inclui também algumas funções utilitárias 
para trabalhar com tipos primitivos. Logo, o sistema pode ser visto como
um complemento para a API padrão de Java. 
...
[`Ints`](https://guava.dev/releases/snapshot-jre/api/docs/) que oferece diversos métodos para trabalhar com valores inteiros. Dentre eles,
temos o seguinte método estático:

```
public static boolean contains(int[] array, int target)
```

Esse método verifica se um vetor de inteiros (`array`) contem um determinado
valor inteiro (`target`).
...
implementa um método que testa o serviço de atualização do nome do dono de um animal:
...
O JUnit possui alguns testes, que, evidentemente, são executados 
pelo próprio JUnit. As primeiras versões de tais testes foram 
implementadas por Kent Beck e Erich Gamma, criadores do JUnit 
e, em grande  medida, do próprio conceito 
